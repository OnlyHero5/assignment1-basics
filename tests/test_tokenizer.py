...
def get_tokenizer_from_vocab_merges_path(
    ...
    for merge_token_1, merge_token_2 in gpt2_bpe_merges:
        try:
            bytes1 = bytes([gpt2_byte_decoder[token] for token in merge_token_1])
            bytes2 = bytes([gpt2_byte_decoder[token] for token in merge_token_2])
            merges.append((bytes1, bytes2))
        except KeyError as e:
            # 如果遇到未知字符，跳过这个 merge
            # print(f"Warning: Skipping merge with unknown character: {e}")
            continue
